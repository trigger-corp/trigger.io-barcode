...
- (void)captureResult:(ZXCapture*)capture result:(ZXResult*)result {
	if (result) {
		...
		[self dismissViewControllerHelper:^{
			...
			// Hacky fix for iOS 7, delegate events seem to come in to self.capture just after the view is hidden, so keep the view alive for a couple of seconds to prevent bad access.
